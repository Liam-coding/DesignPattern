...
public class Level3AuthLink extends AuthLink{
    ...
    public AuthInfo doAuth(String uId, String orderId, Date authDate) {
        ...
        if (date == null) {
            return new AuthInfo("0001", "number: ", orderId, "status: need level3 examine and approve", levelUserName);
        }
        AuthLink next = super.next();
        if (next == null) {
            return new AuthInfo("0000", "number: ", orderId, "status: finished level3 examine and approve","people: ", levelUserName);
        }
